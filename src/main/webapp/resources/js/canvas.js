...
function drawShapes(context) {
    ...
    // прямоугольник
    context.fillRect(CANVAS_CENTER_X, CANVAS_CENTER_Y, R_OFFSET, R_OFFSET);
    // треугольник
    context.moveTo(CANVAS_CENTER_X, CANVAS_CENTER_Y);
    context.beginPath();
    context.lineTo(CANVAS_CENTER_X, CANVAS_CENTER_Y - R_OFFSET / 2);
    context.lineTo(CANVAS_CENTER_X - R_OFFSET / 2, CANVAS_CENTER_Y);
...
aimCanvas.addEventListener('click', e => {
    ...
    let x = Math.round(((canvasX - CANVAS_CENTER_X) * scale *2 ))/2; // округление до 0.5
